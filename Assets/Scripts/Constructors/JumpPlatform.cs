...
public class JumpPlatform : AbstractPlatform
{
    ...
    public override void Move()
    {
        ...
        {
            SpeedDir += (Target1.transform.position - this.transform.position - SpeedDir) * Time.fixedDeltaTime * SpeedGravity* JumpFactor;

            if ((Target1.transform.position - this.transform.position).magnitude < ThreshHold)
                Switch = true;

            Rigid.velocity = SpeedDir * MaxSpeed* JumpFactor;
        }
